...
# ------------------ UART: Send in 4 Quarters ------------------
flat_data = (pin_heights_m / MAX_PIN_DISPLACEMENT_M * 65535).astype(np.uint16).flatten()
quarter_size = len(flat_data) // 4

try:
    with serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=1) as ser:
        time.sleep(2)  # give Arduino time to reset
        print("Sending 32x32 array in 4 quarters and reading Arduino prints...\n")

        for i in range(4):
            start = i * quarter_size
            end = (i + 1) * quarter_size
            print(f"Sending quarter {i+1}...")
            for value in flat_data[start:end]:
                val = int(value)  # convert numpy.uint16 -> Python int
                ser.write((val >> 8).to_bytes(1, 'big'))  # high byte
                ser.write((val & 0xFF).to_bytes(1, 'big'))  # low byte

            # Read Arduino prints for this quarter
            while True:
        ...
        print("\nAll 4 quarters sent and printed!")
except Exception as e:
    print("Failed to send data over UART:", e)
